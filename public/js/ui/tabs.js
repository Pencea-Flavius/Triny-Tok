...
const TAB_LABELS = {
  ...
  aiGameSuggest: 'AI Game Suggestion',
  minecraft: 'Minecraft · Settings',
  commands: 'Minecraft · Gift Commands',
  isaacEffects: 'The Binding of Isaac · Gift Effects',
  repoEffects: 'R.E.P.O. · Gift Effects',
  goiEffects: 'Getting Over It · Gift Effects'
};
...
const TAB_CONSOLE = {
  ...
  goiEffects: 'consoleGoi'
};

const SECTION_LAST_TAB = {};
...
function expandConsole(consoleId) {
  document.querySelectorAll('.sub-console').forEach(other => {
    if (other.id !== consoleId) other.classList.remove('expanded');
  });
...
function activateTab(tabId, { updateUrl = true } = {}) {
  ...
  if (sectionId) SECTION_LAST_TAB[sectionId] = tabId;
...
function activateSection(sectionId, tabOverride, { updateUrl = true } = {}) {
  ...
  // open the first tab
  let tabId = tabOverride || SECTION_LAST_TAB[sectionId] || meta.first;

  // fallback to the very first tab available in the DOM for this section
  if (!tabId) {
    const group = document.querySelector(`.sub-group[data-section="${sectionId}"]`);
    if (group) {
      const firstTabBtn = group.querySelector('.sub-item[data-tab]');
      if (firstTabBtn) tabId = firstTabBtn.dataset.tab;
    }
  }

  if (tabId) {
    SECTION_LAST_TAB[sectionId] = tabId;
    activateTab(tabId);
  }
...
export function setupTabs() {
  ...
  document.querySelectorAll('.sub-console-header').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll('.sub-console').forEach(other => {
        if (other !== consoleEl) other.classList.remove('expanded');
      });
