...
connection.on('like', (msg) => {
    ...
    if (typeof msg.likeCount === 'number') {
        let text;
        if (msg.label) {
            text = msg.label.replace('{0:user}', '').replace('likes', `${msg.likeCount} likes`);
        } else {
            text = `sent ${msg.likeCount} likes`;
        }
        addChatItem('#447dd4', msg, text);
    }
})
...
connection.on('social', (data) => {
    ...
    if (data.label) {
        ...
        text = data.displayType.includes('follow') ? 'followed the host' : 'shared the stream';
    }
